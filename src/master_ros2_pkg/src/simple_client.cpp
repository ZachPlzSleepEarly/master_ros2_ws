...
public:
    StringConcatClient() : rclcpp::Node(NODE_NAME)
    {
        client_ = this->create_client<master_ros2_interface::srv::ConcatStrings>(SERVICE_NAME);

        RCLCPP_INFO(this->get_logger(), "Started ROS2 Service Client");
    }

    void SendRequest(const std::string& str1, const std::string& str2);
...
const std::string StringConcatClient::NODE_NAME = "string_concat_client";

void StringConcatClient::SendRequest(const std::string& str1, const std::string& str2)
{
    ...
    if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), future_result)
        ...
        RCLCPP_INFO(this->get_logger(), "Failed to call service");
    }
}
